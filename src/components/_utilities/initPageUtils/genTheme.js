...
export default baseTheme => {
    ...
    switch (type) {
      case 'maxWidth':
        return `@media (max-width: ${(
          theme.breakpoint.values[pointName] - 1
        ).toString()}px)`;
      case 'minWidth':
        return `@media (min-width: ${theme.breakpoint.values[
          pointName
        ].toString()}px)`;
      case 'minToMaxWidth':
        return `@media (min-width: ${theme.breakpoint.values[
  ...
  for (key of Object.keys(theme.typography.font)) {
