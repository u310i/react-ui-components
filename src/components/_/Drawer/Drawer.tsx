...
const Drawer = ({
	...
	const componentStyle = {
		mask: {
			style: {
				...
				cursor: state === 'close' ? 'auto' : closableOnMask ? 'pointer' : 'auto',
				position: 'fixed',
	...
	useAddCssInBody(name, state === 'open', () => {
		const scrollBarWidth = shiftScrollBarWidth && window.innerWidth - document.body.clientWidth;
		return `
      overflow: hidden;
      ${shiftScrollBarWidth ? `margin-${direction === 'right' ? 'right' : 'left'}: ${scrollBarWidth}px` : ''}
    `;
	});
		...
		<CSSTransition in={state === 'open'} timeout={duration} classNames={name}>
			...
			>
				...
				<div className={cx(css(componentStyle.main.style), 'uc-drawer-main')}>
					<div className={cx(css(componentStyle.main.header.style), 'uc-drawer-main-header')}>
						{closable && (
							<IconButton icon={closeButton.icon} options={closeButton.options} onClick={onClose} />
						)}
					</div>
					<ul className={cx(css(componentStyle.main.list.style), 'uc-drawer-main-list')}>
