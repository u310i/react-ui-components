...
export const cssRgb2Array = (str) => {
	...
	const regExp = new RegExp(`^(rgba?)\\(${rgb},${rgb},${rgb},?\\s*(?:(${percent})|(${float}))?\\s*\\)$`, 'i');
...
export const LightenDarkenHex = (str, amt = 0) => {
	...
	return '#' + ('000000' + (b | (g << 8) | (r << 16)).toString(16)).slice(-6) + hexa.substring(6, 2);
};

export const adjustBrightness = (array, amt = 0) => {
...
export const adjustBrightnessFromCssRgb = (cssRgb, amt) => {
	return toCssColor(adjustBrightness(cssRgb2Array(cssRgb), amt), 'rgba');
};
